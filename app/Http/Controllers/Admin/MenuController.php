...
class MenuController extends MainAdminController
{
    ...
    public function menulist($id)    { 
        ...
        $menu = Menu::where(["restaurant_id" => $id, "status"=>1])->orderBy('menu_name')->get();
    ...
    { 
		...
		$menu->menu_type = $inputs['menu_type'];
		// $menu->opening_time = $inputs['opening_time'];
		// $menu->closing_time = $inputs['closing_time'];
		$menu->delivery_charge = $inputs['delivery_charge'];
		$menu->status = $inputs['status'];
		
        $menu->description = $inputs['description'];
		$menu->availablity = implode(",",$inputs['availablity']);
		$menu->days_avail = implode(",",$inputs['days_avail']);
		$menu->trail_price = $inputs['trail_price']; 
		$menu->is_trail = $inputs['is_trail']; 
        $menu->price = $inputs['price']; 
		// $menu->weekly = $inputs['weekly']; 
		// $menu->monthly = $inputs['monthly']; 
		// $menu->quarterly = $inputs['quarterly']; 
		$menu->order_min_count = $inputs['order_min_count']; 
		$menu->order_max_count = $inputs['order_max_count']; 
    ...
    {    
    	if(Auth::User()->usertype=="Admin" or Auth::User()->usertype=="Merchant")
        {
            ...
            //new code for delete menu (hiren)
    ...
    { 
        ...
        $menu = Menu::where(["restaurant_id" => $restaurant_id , "status"=>1])->orderBy('menu_name')->get();
